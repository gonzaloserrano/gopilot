...
Use `golangci-lint` with errcheck, govet, staticcheck, gocritic, gofumpt, wrapcheck, errorlint enabled. Before committing, check for Makefile targets (`make lint`, `make test`, `make build`); fallback: `go build/test/lint/fix/fmt` + `go mod tidy`.

See [Linting & Pre-Commit reference](reference/linting.md) for full golangci-lint config and pre-commit checklist.
